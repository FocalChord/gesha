...
public class ParsingSolarData {
    ...
    public ParsingSolarData(String ip, int port) {
        ...
        this.filePath = "/Users/nisa/Desktop/P4P/iec61850bean/solar.csv";
        this.filePath_house = "/Users/nisa/Desktop/P4P/iec61850bean/house.csv";
    }
    ...
    public String getRandomTemperature(int min, int max) {
        int range = max - min + 1;
    ...
    public boolean writeSensor(int lineIndex) throws IOException {
        ...
        PrintWriter out = new PrintWriter(socket.getOutputStream(), true);
        BufferedReader input = new BufferedReader(new InputStreamReader(socket.getInputStream()));
        String command = "SSEN " + getRandomTemperature(12, 32);
        out.println(command);
        String serverResponse = input.readLine();
        System.out.println("Server says: " + serverResponse);
        socket.close();
